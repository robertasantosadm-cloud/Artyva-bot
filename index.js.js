...
// Numero autorizado da Roberta (corporativo)
...
async function processMessage(from, text) {
  ...
  if (pausados[from]) {
    ...
    if (tempoPassado > TEMPO_PAUSA_MS) {
      // 3h passaram — reativa com boas-vindas
      pausados[from] = false;
  ...
  // ── CLIENTE RECORRENTE (3h+ sem interagir) ────────────
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    // ── COMANDOS DA ROBERTA ──
    // Identifica se veio do numero corporativo da Roberta
    const eRoberta = from === NUMERO_ROBERTA || fromMe;

    if (eRoberta && msg === '#pausa') {
      // #pausa sem numero = pausa o proprio contato (uso interno)
      console.log('COMANDO #pausa recebido de Roberta');
      return res.sendStatus(200);
    }

    // Roberta responde manualmente para um cliente = pausa aquele cliente
    if (fromMe && from && text !== '') {
      if (msg === '#bot') {
        // Roberta ativa #bot = inicia contagem de 3h para reativar
        pausados[from] = true;
        ultimaPausa[from] = Date.now();
        console.log('Bot aguardando 3h para reativar:', from);
      } else {
        // Roberta respondeu normalmente = pausa imediato
        pausados[from] = true;
        ultimaPausa[from] = Date.now();
        console.log('Bot PAUSADO (Roberta respondeu) para:', from);
      }
      return res.sendStatus(200);
    }

    // Mensagem do numero corporativo com #pausa = pausa o contato especificado
    if (from === NUMERO_ROBERTA) {
      if (msg === '#pausa') {
        console.log('IGNORADO - #pausa do corporativo sem contexto');
        return res.sendStatus(200);
      }
      if (msg === '#bot') {
        console.log('IGNORADO - #bot do corporativo sem contexto');
        return res.sendStatus(200);
      }
    }
