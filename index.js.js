...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    // ── Aceita tanto payload nativo (Events) quanto Evolution Bot ──
    let remoteJid = '';
    let fromMe    = false;
    let text      = '';

    if (body.event === 'messages.upsert' && body.data) {
      // ── PAYLOAD NATIVO DO EVOLUTION (Events/Webhook) ──
      const data = body.data;
      remoteJid  = String(data.key?.remoteJid || '').trim();
      ...
      // ── PAYLOAD EVOLUTION BOT (legado) ──
      const inp = body.inputs;
      remoteJid  = String(inp.remoteJid || inp.user || '').trim();
      fromMe     = inp.fromMe === true || inp.fromMe === 'true';
      text       = String(body.query || inp.query || inp.text || '').trim();
    } else {
      console.log('[IGNORADO] payload desconhecido');
      return;
    }
